...
> Phase 3-A1 允许保存用户提供的 URL；Phase 3-A2 允许校验和保存用户上传的本地栅格图片；Phase 3-A3 允许基于本地文件名生成并手动确认媒体关联候选。仍禁止请求外部网页、远程图片、爬虫、站点 adapter、自动同步、识别、推荐、AI、多用户或云同步。
...
产品功能重启：Phase 3-A1 来源导入、A2 本地媒体库与 A3 本地媒体候选配对已完成，尚未发布
```
...
核心业务能力：已完成
代码发布状态：v1.0.4 已发布
当前开发状态：Phase 3-A3 已完成，main 仍使用 Schema 2
WSL 验收：已完成
...
12. 当前 main 仍为 Schema 2；稳定发布版仍为 v1.0.4
