...
> Phase 3-A1 允许保存用户提供的 URL；Phase 3-A2 允许校验和保存用户上传的本地栅格图片；Phase 3-A3 允许基于本地文件名生成并手动确认媒体关联候选；Phase 3-A4 允许从未匹配本地图片手动确认创建条目；Phase 3-A5 允许对完整本地媒体扫描结果进行只读检索与分页；Phase 3-A6 允许在数据健康页只读审计本地媒体完整性；Phase 3-B1 允许按完整 SHA-256 只读定位重复媒体。仍禁止请求外部网页、远程图片、爬虫、站点 adapter、自动同步、识别、推荐、AI、多用户或云同步。
...
产品功能重启：Phase 3-A1 至 A6 已随 v1.0.5 发布；Phase 3-B1 重复媒体定位已在 main 完成，尚未发布
```
...
核心业务能力：已完成
代码发布状态：v1.0.5 已正式发布
当前开发状态：Phase 3-B1 已完成本地验收并保留在 Unreleased，main 仍使用 Schema 2
WSL 验收：已完成
...
11. Phase 3-B1 重复媒体定位已在 main 完成，尚未发布
12. 当前稳定版为 v1.0.5，应用 Schema 为 2
...
- 当前 B1 代码开发与本地测试已完成；N100 部署不是当前开发任务，须等待用户明确授权
