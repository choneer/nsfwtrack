...
> Phase 3-A1 允许保存用户提供的 URL；Phase 3-A2 允许校验和保存用户上传的本地栅格图片；Phase 3-A3 允许基于本地文件名生成并手动确认媒体关联候选；Phase 3-A4 允许从未匹配本地图片手动确认创建条目；Phase 3-A5 允许对完整本地媒体扫描结果进行只读检索与分页。仍禁止请求外部网页、远程图片、爬虫、站点 adapter、自动同步、识别、推荐、AI、多用户或云同步。
...
产品功能重启：Phase 3-A1 来源导入、A2 本地媒体库、A3 媒体候选配对、A4 未匹配媒体建档与 A5 媒体检索分页已完成，尚未发布
```
...
核心业务能力：已完成
代码发布状态：v1.0.4 已发布
当前开发状态：Phase 3-A5 已完成，main 仍使用 Schema 2
WSL 验收：已完成
...
14. 当前 main 仍为 Schema 2；稳定发布版仍为 v1.0.4
15. N100 / 目标主机部署未开始，等待用户明确授权
16. 其余仅按实际问题做可选维护
```
