...
```text
v1.1.0 stable / Phase 5-N4C provider direction research locally complete
```
...
下一目标：v1.2.0 三类 Provider 路线；Phase 5-N4A/N4B 通用基础设施与 machine-checkable Approval gate、N4C 静态研究和 placeholder Approval 草案已完成，N4D/N4E/N4F/N4G 仍分别等待完整 Provider Approval
```
...
当前开发状态：Phase 5-N4D-B Video Metadata DTO 与 Fixture Adapter Framework 已完成；应用仍为 `1.1.0`、Schema 为 4、Backup 为 `nsfwtrack.backup.v2`；Production Registry 为空，无真实 Provider，fixture adapter 仅位于 tests。
...
- 公开研究快照为 JavdBviewed `e26dfdf...`、JavSP `c4cfe611...`、FnDepot
  `e565623...`、Venera `a0eba914...`；只记录架构，不复制实现
...
- Approval format 继续为 `1`，新字段使用现有行为兼容的 deny-safe 默认值；Production
  Registry 仍为空，N4D-B 才能进入完整真实 Video Metadata Approval/Adapter
