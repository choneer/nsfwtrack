...
> Phase 3-A1 允许保存用户提供的 URL；Phase 3-A2 允许校验和保存用户上传的本地栅格图片；Phase 3-A3 允许基于本地文件名生成并手动确认媒体关联候选；Phase 3-A4 允许从未匹配本地图片手动确认创建条目；Phase 3-A5 允许对完整本地媒体扫描结果进行只读检索与分页；Phase 3-A6 允许在数据健康页只读审计本地媒体完整性。仍禁止请求外部网页、远程图片、爬虫、站点 adapter、自动同步、识别、推荐、AI、多用户或云同步。
...
```text
v1.0.5（已冻结并在 main 准备，尚未创建 tag / GitHub Release）
```
...
产品功能重启：Phase 3-A1 来源导入、A2 本地媒体库、A3 媒体候选配对、A4 未匹配媒体建档、A5 媒体检索分页与 A6 媒体完整性审计已冻结至 v1.0.5 发布准备
```
...
```text
核心业务能力：已完成
代码发布状态：v1.0.4 已发布；v1.0.5 候选已准备，tag / Release 尚未创建
当前开发状态：Phase 3-A1 至 A6 已冻结为 v1.0.5，main 仍使用 Schema 2
WSL 验收：已完成
...
代码开发与 WSL 验收已完成至 `v1.0.5` 发布候选。N100 / 目标主机部署尚未开始，
必须等待用户明确授权后才能执行。
...
### Phase 3-A1 — 来源链接与本地书签导入（已纳入 v1.0.5 发布准备）
...
### Phase 3-A2 — 本地媒体库（已纳入 v1.0.5 发布准备）
...
### Phase 3-A3 — 本地媒体候选配对（已纳入 v1.0.5 发布准备）
...
### Phase 3-A4 — 未匹配媒体快速建档（已纳入 v1.0.5 发布准备）
...
### Phase 3-A5 — 媒体库检索与分页（已纳入 v1.0.5 发布准备）
...
### Phase 3-A6 — 本地媒体完整性审计（已纳入 v1.0.5 发布准备）
...
### v1.0.5 — Phase 3-A1 至 A6 发布准备（已完成，待 tag / Release）
...
- 不移动旧 tag / Release，本轮不创建 v1.0.5 tag 或 GitHub Release
- 不部署到 N100
...
9. Phase 3-A1 至 A6 已冻结并纳入 v1.0.5 发布准备
10. 来源同名歧义与媒体原子上传修复已纳入 v1.0.5 发布准备
11. 当前 main 应用版本为 1.0.5、Schema 为 2；最新已发布版仍为 v1.0.4
12. v1.0.5 tag / GitHub Release 本轮不创建
13. N100 / 目标主机部署未开始，等待用户明确授权
14. 其余仅按实际问题做可选维护
```
