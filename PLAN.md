...
```text
Application 1.3.0 / Phase 6-R4 formally released
```

Phase 6-R4 已正式发布 Application `1.3.0`，Schema 保持 `5`。统一持久任务模型、
受控资产下载、手动来源 Check/Diff/Confirm/Apply、双语任务中心及其迁移/事务/
安全测试已经 complete/frozen；R3 候选、Cloud RC diff review 与唯一一次 Hermes
acceptance 均 PASS。production endpoint/package/provider/acquisition catalog 全部
保持空，不发布镜像、不部署 N100，R4 未再次调用 Hermes。

当前最新稳定版本为 `v1.3.0`；`v1.2.0` 与 `v1.1.0` 发布证据作为历史保留。
...
正式发布：Application `1.2.0` 已随 Phase 5-R4 发布；Phase 5-N4A/N4B 通用基础设施、N4C 静态研究、N4D-A/B/C、N4D-D-A Artifact 离线门禁、N4D-D-B0 固定仓库证据 profile、N5A/N5B 与完整 N5C Signed Preview/Confirm/local Apply 闭环已完成；N4D-D-B/N4E/N4F/N4G 仍分别等待完整 Provider Approval
```
...
```text
核心业务能力：已完成
代码发布状态：v1.3.0 已按 Phase 6-R4 发布门禁正式发布；v1.2.0/v1.1.0 tag/Release 证据保留为历史
当前发布状态：Phase 5-N5C-B2 的 Session-bound Search → Detail → signed Preview → explicit Confirm → local Apply 基础保持冻结；Phase 6 的持久任务、受控下载、手动更新和 Task Center 已 complete/frozen。Application `1.3.0` 已正式发布且 latest stable 为 `v1.3.0`，Schema 为 `5`、Backup 为 `nsfwtrack.backup.v2` 且 restore 接受 v1/v2，Production Endpoint Registry、Search Packages、Search Providers 与 Acquisition Registry 均为空。
...
N5C-B1 已完成重验与事务写入服务，N5C-B2 已完成显式确认页面集成。N5A 本地门禁为 focused `33 passed`、targeted
`376 passed`、full `1194 passed`。

### Phase 5-N5B：Search/Detail Empty-State and Approved-Provider UI（已完成）

N5B 新增登录保护的 `/source-search` 页面和显式 Search/Detail POST。生产依赖只
构造 `build_production_search_service()`，测试通过 FastAPI dependency override
注入 tests-only Service。GET 只调用 `list_providers()`；生产 catalog 为 `()` 时
返回普通 200 空状态“暂无可用外部来源”，不加载 synthetic Provider，也不触发
Search、Detail、Asset List、Outbound、DNS、数据库或文件写入。

Search 与 Detail 分别通过 N5A request contract 验证，并对当前 descriptor catalog
和批准 operation 做前置复核；每个显式 POST 只调用对应 Service operation 一次，
Search 不自动 Detail，Detail 不自动 Asset List。结果仅由 Jinja 自动转义展示；
canonical URL 不成为链接，cover/preview/asset 不成为远程图片、播放或下载入口，
Detail 只显示 Asset kind/name/MIME/尺寸/时长等非 locator facts。Service errors 映射
为稳定、脱敏的 400/409/502/503 i18n 状态，取消保持传播，响应不写 DB、Session、
Cookie、文件或缓存。

N5B 新增中英文导航、模板和 `38` 个专项测试；N5A+N5B 组合 `71 passed`，认证/
...
N5C-A 当时冻结的独立强制合同现已由 N5C-B1 服务层落实：验签后不得重新调用
Provider，必须在 `BEGIN IMMEDIATE` 后重读 identity source、URL source、linked Item
和 duplicate-title IDs，与 snapshot 逐项比较；任何变化返回 `stale_plan` 且零写入。
最终写入在单一事务中完成，commit 后由独立 Session 证明 bounded result，Token 重放
因状态变化失败。签名有效不等于状态有效；N5C-B2 页面确认集成现已完成且仍依赖 B1 锁内重验。
...
N5C-B1 新增纯服务层 `apply_provider_apply_token`。入口先完成既有 purpose-bound
Signed Token 验证，之后才检查 Session/factory 门禁并执行 SQLite
`BEGIN IMMEDIATE`；任何 identity source、normalized URL source、linked Item 或
duplicate-title ID 查询都不得早于写锁。锁内重读使用稳定 `ORDER BY id ASC LIMIT 2`
和 `ORDER BY id ASC LIMIT 32`，任一 snapshot 变化返回 `stale_plan` 且零业务写入。

create 只创建一个 Item 和一个 ItemSource；update 只允许 Plan 中
`will_write=True` 的 Item.summary、Item.release_date、ItemSource.last_checked_at 与
ItemSource.metadata_hash。Item title、cover、extra、Source URL/title/identity 与所有
关系表均不修改，不按标题绑定已有 Item。flush 后、commit 前再次证明 exact
transaction post-state；commit 正常返回也必须由独立 Session 证明 durable post-state。

异常路径先独立证明 post-state，再证明 pre-state：前者返回
`committed_verified_after_exception`，后者按 Integrity/其他失败稳定分类为
`write_conflict` / `write_failed`，两者都不能证明时返回 `commit_state_unknown`。
成功 create/update 的同 Token 重放均为 `stale_plan`。B1 不新增 Router、页面、
Secret/context 派生、Provider 调用、网络、依赖、Schema、Backup、Docker 或 CI 变化；
N5C-B2 现已负责并完成 Preview/Confirm UI、session-bound secret/context、模板与 i18n。
本地门禁为 focused `47 passed`、指定 N4D/N5A/N5B/N5C 组合 `287 passed`、full
`1352 passed`；`pip check` 与 diff/status 门禁在提交前再次执行。

### Phase 5-N5C-B2：Session-Bound Preview/Confirm UI（已完成）

`app/provider_apply/web.py` 仅从现有已认证 Session 派生 Web key material。Preview
按需创建 64 位 lowercase-hex nonce；Confirm 只能读取既有 nonce。当前 Session
generation 必须与 app generation 精确恒定时间匹配。`SECRET_KEY`、generation 与 nonce
使用两个独立 domain 的 HMAC-SHA256，分别派生 exact 32-byte secret 和 bounded opaque
context；这些值不进入 Session、模板、URL、flash、日志或数据库。

Detail POST 仍只调用一次 Provider detail，随后只用 N5C-A bounded SELECT 构建 Plan，
零数据库写入。有变化时签发固定 600 秒 Token，Token 只出现在 `autocomplete=off` 的
hidden input，响应为 `no-store`；no-op 不签发 Token、不显示表单且不创建 nonce。页面只
显示允许的确定性本地字段、keep-local 决策和 duplicate-title 本地链接，不显示 canonical
URL、external ID、metadata hash 或任何 key material。

Confirm POST 受 `require_page_auth` 保护并精确要求 `confirmation=apply`，不读取 Provider
catalog，不调用 search/detail/asset_list，每请求最多调用 B1 apply 一次。成功按 303 PRG
跳转 Item；普通失败回到 source search。`commit_state_unknown` 跳转 Item 列表，明确要求
先检查本地条目且禁止直接重试。跨浏览器 Session、logout/relogin、generation rotation、
缺失 nonce 均在业务写入前拒绝旧 Token。

N5C-B2 focused `36 passed`、指定 N4D/N5A/N5B/N5C 组合 `323 passed`、full
`1388 passed`。N5C 完成 Search → Detail → signed Preview → explicit Confirm → local
Apply；Production Registry、Search Packages 与 Search Providers 继续为空，没有真实
Provider、认证/Vault、播放、下载、后台任务、Schema、Backup、依赖、Docker 或 CI 变化。
...
16. Phase 5-N5C-B1 Transactional Provider Apply Service；已完成，纯服务层
17. Phase 5-N5C-B2 Preview/Confirm routes、session-bound secret/context 与 UI；已完成
18. Phase 5-N6 用户明确确认的受控资源保存与下载；未实现
19. Phase 5-N7 多来源更新、受控同步和推荐；未实现
20. Phase 5-R1 v1.2.0 静态集成冻结审计；已通过
21. Phase 5-R2 仅在 R1 发现阻塞时进行 corrective；本次不需要
22. Phase 5-R3 Application `1.2.0` 更新、RC 冻结与冻结后唯一一次 Hermes 验收
23. Phase 5-R4 正式发布；尚未发布且需单独授权
24. N100 / 目标主机部署仍未授权，不属于本路线
```
...
- Phase 4 已随 v1.1.0 发布；Phase 5-N1 至 N5C-B2 的已授权基础、UI、只读 signed
  plan、事务服务层与 Session-bound Preview/Confirm 闭环均已完成，Production
  Provider/Search catalog 仍为空
