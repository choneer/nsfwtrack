...
- [x] **6.10 Lease fencing** — 每个 chunk 写入与 progress commit 前后，以及
  integrity/publication/DB link/index/final success 前均验证 running、owner、
  generation、未过期与未取消；heartbeat 采用 generation 条件原子续租
- [x] **6.11 并发与真实竞态** — Start/Resume 原子条件 claim 不超过上限；
  pause 后立即 resume 的旧执行器停在首个已提交 chunk，只有新 generation 完成
  唯一文件、ItemLocalAsset、索引和成功事件
- [x] **6.12 下载独立证明** — `durable_verified` 前后均由新 Session 验证精确
  task/version/link、目录 FD + `O_NOFOLLOW` 文件身份/SHA/size 与 media index；
  commit/读取/DB link/index 证明失败按 committed/write-failed/unknown fail closed
- [x] **6.13 手动更新独立证明** — 新 Session 同时验证选中字段、未选字段、来源
  identity、task intent/snapshot/version/events 与 replay 唯一性，只有完整证明才返回
  `committed_verified`

集中修正验证：Phase 6 `45 passed`，媒体/迁移/备份/发布安全组合 `160 passed`，
完整测试 `1453 passed in 607.60s`。隔离 Docker 双生命周期验证 Schema 5、
owner/generation fencing、restart paused recovery、generation 1 → 2、文件/DB/索引
identity/SHA/size 及 SQLite/FK 一致；临时容器、镜像与审计目录均已清理，仓库
`data/` 未接触。
...
- [x] **12.M3.2. 旧数据兼容** — Schema 2 条目、来源和其他业务数据是否原样保留，全新数据库是否直接建立 Schema 3 与空失效索引
- [x] **12.M3.3. 派生缓存边界** — 索引是否不进入备份、不从备份恢复，恢复是否在同一事务中标记索引失效，损坏索引是否不影响业务数据或媒体文件
- [x] **12.M3.4. 缓存命中条件** — 是否仅在签名、mode、size、dev、inode、mtime、ctime 与完整父目录映射一致时复用 SHA / MIME / validity
- [x] **12.M3.5. FD 安全语义** — 增量扫描是否沿用 O_NOFOLLOW 目录 FD 遍历、候选打开、读取前后身份复核和最终路径映射复核
- [x] **12.M3.6. 变化处理** — 新增、删除、损坏、skip、父目录替换、inode 替换、identity 变化和伪造缓存是否有确定结果与测试
- [x] **12.M3.7. 原子刷新** — 新快照是否完整生成后才在单一事务中替换，扫描中途或提交失败时是否保留上一份完整索引
- [x] **12.M3.8. 完整验证** — 是否忽略所有缓存内容事实并重新读取、校验、哈希普通媒体，可从空表或损坏索引恢复
- [x] **12.M3.9. 扫描中心** — 是否登录保护，GET 零写入，状态、时间、耗时、文件统计、变化统计、错误与变化明细是否完整
- [x] **12.M3.10. 手动触发** — 是否只有显式增量 POST 和完整验证预览 / 确认 POST，没有后台线程、队列、定时或启动扫描
- [x] **12.M3.11. 只读接入** — 媒体库、目录、重复组、alias、匹配候选是否优先使用完整索引并显示快照时间与可能过期状态
...
## Phase 3-D1 Unreleased 最终集成审查

- [x] **12.661. 范围是否完整** — B3-B6、C1-C5 的路由、服务、模板、导航、状态闭环和回归均已逐项审查
- [x] **12.662. finding 是否闭环** — 每类 Data Health finding 均有直接入口、明确只读位置 / 说明或安全拒绝，处理后消失 / 保留逻辑已记录
- [x] **12.663. 路由与死链接是否清零** — 19 个相关路由全部登录保护；110 个注册路由与 142 个模板字面量链接核对为 0 缺失
- [x] **12.664. GET 是否零写入** — 列表、诊断、预览、重复组、恢复中心和跳过项的数据库 / 文件系统不变测试均通过
- [x] **12.665. 写操作是否受控** — 所有变更均为登录 POST、浏览器 / 服务端确认，strict 精确 `CONFIRM`，并具备事务 / 写锁、身份重验和准确失败报告
- [x] **12.666. Data Health 读取边界是否关闭** — 根目录、未扫描引用和上传残留不再通过可替换路径独立遍历，外部 symlink 目标不读取
- [x] **12.667. B3-B6 父链是否身份绑定** — 验证、锚点创建、恢复发布和删除均保留 `O_DIRECTORY|O_NOFOLLOW` 父目录 fd 链并复核当前映射
- [x] **12.668. C2 父链是否身份绑定** — 残留观察保存父链身份，最终 unlink 前重开并精确复核父链与最终文件映射
- [x] **12.669. 外部目录项是否保持** — 父目录改名 / symlink 与外部同 inode 硬链接竞态下，原文件、移动目录及外部链接均不被误删或覆盖
...
## Phase 4-A1 本地媒体单文件详情检查

- [x] **12.678. 登录与方法边界** — `/media-library/detail` 是否要求登录、只提供 GET，POST 是否为 405
- [x] **12.679. 路径边界** — 是否只接受规范化 `/media/` 精确普通扫描记录，并拒绝外部、转义、missing、symlink、目录 / FIFO、unsupported 和 cleanup anchor
- [x] **12.680. 安全读取复用** — 是否直接消费现有逐级目录 / 文件 FD、内容和映射复验结果，且不通过目标 `Path.stat` / `Path.read_bytes` 二次打开
- [x] **12.681. 文件事实完整** — 是否展示逻辑路径、文件名、扩展名、MIME、size、完整 SHA、有效 / 损坏和 recovered 状态
- [x] **12.682. 引用完整** — 是否精确展示全部 item cover / creator avatar 引用、对象链接和引用 / 未引用状态
- [x] **12.683. 重复组准确** — 是否按同次扫描的完整 SHA 展示成员数、单文件 / 总占用 / 可释放空间、成员路径及准确组入口
- [x] **12.684. 写操作边界** — 损坏文件 / 引用是否仅链接现有 C4 / C1，详情页是否未新增删除、替换、移动、重命名或批量写操作
- [x] **12.685. 来源状态返回** — 媒体库、重复组、recovered 普通媒体入口是否保留各自规范化搜索、状态、排序和页码
- [x] **12.686. 零写入与竞态** — GET 是否无 SQL 写语句且文件 / 目录不变，父目录替换时是否只读原 FD 后 404、不读取外部目标
- [x] **12.687. 双语与局部回归** — 中文 / English 是否对称，A1 专项 17 passed、媒体 / 恢复 / C1/C4 / Data Health / 备份 / UI 组合 252 passed
- [x] **12.688. 最终交付** — 专项 17、媒体链 252、全量 601、pip check 与隔离 Docker / HTTP 已通过；实现提交 `c8cfb99` 已推送，Actions run `29389862206` 两个 job 均成功
- [x] **12.689. 范围保持** — 版本 1.0.6、Schema 2、迁移、依赖、Docker/CI、tag、Release 与 N100 是否不变

本地证据：A1 专项 `17 passed`；媒体、恢复、C1/C4、Data Health、备份、
页面、响应式与 i18n 组合 `252 passed in 38.99s`；全量
`601 passed in 91.96s`；`pip check` 无冲突。Docker image build 通过，
隔离 Compose 为 healthy 且保持 user `10001:10001`、read-only root、
`cap_drop: ALL` 与 `no-new-privileges`；`/login` 200、匿名详情 303、认证
详情 / 媒体库 200，临时容器、网络、cookie、夹具和数据已清理。
实现提交 `c8cfb99` 已推送；GitHub Actions run `29389862206` 的
`test` 与 `Docker production smoke` 均为 success。

## Phase 4-A2 普通媒体安全重命名检查

- [x] **12.690. 入口与资格** — A1 是否只为有效普通 / recovered 媒体提供登录保护入口，anchor、residue、damaged、skip、symlink 和特殊文件是否排除
- [x] **12.691. basename 边界** — 是否只接受同目录 basename、原扩展名与大小写，并拒绝路径段、控制字符、百分号、保留前缀、过长或未变化名称
- [x] **12.692. GET 零写入** — 预览是否展示源 / 目标、完整 SHA、mode / size / dev / inode / mtime / ctime、全部引用与后果，且 SQL、文件和目录均不写
- [x] **12.693. 目标 no-overwrite** — 文件、symlink、目录、FIFO、同 inode hardlink、其他对象和目标已有引用是否都在覆盖前拒绝
- [x] **12.694. 确认与快照** — POST 是否复用 standard / strict `CONFIRM`，并拒绝伪造 / 过期身份或引用 ID 集合
- [x] **12.695. 写锁重验** — 是否在 `BEGIN IMMEDIATE` 内重验源完整身份、目标不存在且零引用、全部源引用
- [x] **12.696. FD 硬链接** — 是否通过保持打开的已验证父目录 FD no-overwrite 建立目标，并持续复核源 / 目标 FD 与当前父链映射
- [x] **12.697. 引用事务** — 是否迁移全部 cover / avatar，精确复核 rowcount、源零引用和目标 ID 集合后才 commit
- [x] **12.698. DB 失败补偿** — 提交前失败是否 rollback 原引用并按 held-FD inode 删除自建目标，且不删除目标抢占者 / 外部对象
- [x] **12.699. commit 后删除** — 是否在第二个写锁复核后按身份 unlink 源；失败时保持有效目标 / 引用并准确报告源或双路径状态
- [x] **12.700. 竞态矩阵** — 目标抢占、源 / 目标替换、普通目录 / symlink 父替换、同 inode link、引用变化、commit 与 unlink 失败是否覆盖
- [x] **12.701. 内容关系保持** — SHA、内容、inode、重复组和非路径元数据是否保持，成功后是否返回保留来源状态的新详情
- [x] **12.702. 局部回归** — A2 专项 43 passed，local-media / B3 / B5 / C1/C2/C4 / A1 / i18n 组合 144 passed，媒体 / Data Health / 备份 / UI 组合 309 passed
- [x] **12.703. 最终交付** — 全量 644 passed、pip check 与隔离 Docker / HTTP 已通过；实现提交 `b32e848` 已推送，Actions run `29396021693` 两个 job 均成功
- [x] **12.704. 范围保持** — 版本 1.0.6、Schema 2、迁移、依赖、Docker/CI、tag、Release 与 N100 均未改变
- [x] **12.705. commit 结果复核** — `db.commit()` 抛错后是否先 rollback 当前 Session 释放锁，再用新的独立 Session 查询 source / target 全部 cover / avatar 引用
- [x] **12.706. 唯一清理条件** — 是否仅在全部预期引用仍精确位于 source 且 target 零引用时，才按 held-FD inode 清理自建 target
- [x] **12.707. 已提交与未知结果** — 全部引用位于 target 时是否保留双路径并返回 `committed_source_retained`；混合、无引用歧义或查询失败是否返回 `commit_outcome_unknown` 且不删除
- [x] **12.708. 引用始终可用** — 真实 commit 后抛错、混合引用与查询失败测试是否同时包含 item cover / creator avatar，并确认每条引用均解析到存在的有效媒体
- [x] **12.709. 修复最终交付** — 修复后专项 50 passed、核心媒体链 193 passed、广泛组合 315 passed、全量 650 passed、pip check 与隔离 Docker / HTTP 已通过；提交 `09be556` 与 Actions run `29399210087` 两个 job 均成功

当前本地证据：A2 专项 `43 passed`；既有 local-media、B3、B5、C1/C2/C4、
A1 与 i18n 回归 `144 passed in 26.99s`；媒体、Data Health、备份、页面、
响应式与 i18n 组合 `309 passed in 56.52s`；全量
`644 passed in 119.82s`；`pip check` 无冲突。Docker image build 通过，
隔离 Compose healthy 且保持 user `10001:10001`、read-only root、
`cap_drop: ALL` 与 `no-new-privileges`；`/login` 200、匿名 rename 303、
API login / 认证媒体库 200，临时容器、网络、cookie 和数据已清理。
实现提交 `b32e848` 已推送；GitHub Actions run `29396021693` 的
`test` 与 `Docker production smoke` 均为 success。

commit 歧义修复本地证据：真实 `original_commit()` 后抛错会保留双路径，
并通过独立 Session 确认 item cover / creator avatar 全部位于 target；混合引用、
无引用歧义及独立查询失败均保留双路径且不显示迁移成功。修复后 A2 / i18n
专项 `50 passed`，核心媒体链组合 `193 passed in 27.83s`。
广泛媒体 / Data Health / 备份 / UI 组合 `315 passed in 46.38s`；全量
`650 passed in 106.83s`；`pip check` 无冲突。隔离 Docker image build、
Compose healthy、运行安全边界与 `/login` 200 均通过，临时资源已清理。
...
- [x] **12.711. 目录浏览状态** — 面包屑、子目录、文件 / 大小 / 损坏 / 重复 / 未引用统计、搜索、状态、排序、分页和返回状态是否完整
- [x] **12.712. 目录与别名零写入** — 两个 GET 页面是否要求登录、捕获 SQL 写为 0，并且不创建 / 删除 / 重命名目录或媒体
- [x] **12.713. 共享安全引擎** — 同目录重命名与跨目录移动是否共享 FD、身份、hardlink、事务、commit 歧义和清理实现
- [x] **12.714. 跨目录 no-overwrite** — 现有任何文件、hardlink、symlink、目录、FIFO、引用或竞态抢占是否阻止覆盖
- [x] **12.715. 目录链快照** — POST 是否比较预览时源 / 目标完整目录链 token，并在父目录替换时安全拒绝
- [x] **12.716. 引用迁移完整** — 移动是否精确迁移全部 item cover 与 creator avatar，复核 rowcount、源零引用及目标 ID 集合
- [x] **12.717. 移动失败语义** — 预提交失败是否回滚且仅清理自建目标，未知提交或删除失败是否保留有效安全路径
- [x] **12.718. 单项引用预览** — 是否明确展示对象、字段、旧路径、新路径、set / replace / clear 和后果，GET 保持零写入
- [x] **12.719. 单字段更新** — POST 是否使用 standard / strict 确认、对象 token、媒体完整身份和条件 SQL，只修改指定引用字段
- [x] **12.720. 引用 commit 歧义** — 独立查询是否区分 committed / not committed / unknown，并且对象不存在不与 NULL 字段混淆
