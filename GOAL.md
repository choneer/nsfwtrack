...
- 不跟随符号链接，不读取符号链接目标
- 媒体候选读取必须从根 fd 逐段验证父目录，最终文件仅通过 `dir_fd + O_NOFOLLOW` 打开
- 遍历时保存根、父目录及最终文件的 dev / inode / size / mtime / ctime，读取前后和当前路径映射均须一致
- 父目录后置替换、文件替换或身份漂移必须安全跳过，外部替换内容不得读取、解析或哈希
- 不读取、解析或哈希被跳过文件的内容
...
- 候选读取不使用可重新解析父路径的 `Path.stat/read_bytes`
- 子目录 fd 打开后父路径替换为外部 symlink 时，同名外部合法图片仍零读取、零哈希且不进入 entries
- 不读取任何被跳过文件的内容
- 单个目录或条目失败不会中断整个扫描
- 汇总数量与逐项列表一致
- 搜索、筛选、排序和分页状态正确保留
...
- C3 专项：已扩展至 `10 passed`，包含父目录后置替换和同 inode 身份漂移竞态
