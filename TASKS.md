...
- [x] 统一 `asset_download` / `source_check` / `metadata_update` 持久任务、
  closed transition matrix、乐观版本、lease generation/heartbeat/expiry、
  redacted error、保守重启恢复和显式 history 删除
- [x] Provider-neutral acquisition contract 与固定空 production acquisition
  registry；测试只使用 synthetic adapter，不访问真实 DNS/站点
- [x] 纯 Preview → Session-bound signed Confirm → 单一 queued Task → 显式
  Start/Resume → mode-600 temp → stream/magic/MIME/SHA/size 验证 → 无覆盖发布
  → 事务链接 → media-index coordination 完整闭环
- [x] 手动来源 Check 只调用一次 detail；Diff/逐字段选择/签名计划/Confirm
  零 Provider 调用并只写白名单字段，Item title 与全部用户状态保持本地所有权
- [x] `/tasks` 双语分页/筛选/list/detail，以及 Start/Pause/Resume/Cancel/
  safe Retry/delete-history；GET 不执行任务、不调用 Provider
...
写入、事务 post-check、独立 Session 最终状态证明与 replay rejection；N5C-B2 已完成
Session-bound Preview/Confirm route、HMAC domain-separated secret/context、模板、i18n、
...
- [x] 审查 JavdBviewed `8c924572...`、JavSP `c4cfe611...`、FnDepot
  `9a2449ea...`、Venera `a0eba914...` 的默认分支、准确 SHA、许可证和参考文件
- [x] 定义影视统一 DTO、`search`/`detail`/`asset_list`、字段 provenance、
  用户字段优先、缺失不删除、空值不覆盖与 deterministic merge
- [x] 定义 Subscription Catalog/Candidate/Revision/Diff/Validation、主动 POST
  refresh、候选 review/disable；候选 `baseUrl` 不访问且 `premium` 不授予权限
- [x] 只设计 PlaybackGroup/Source/Variant/Manifest/Segment、播放与下载状态机，
  未实现真实播放、媒体访问或下载
- [x] 定义漫画 search/category/detail/chapter/page/asset DTO 与阅读流；未来来源
  固定为审查过的 Python Adapter，禁止远程 JavaScript/Source 执行
- [x] 三份研究均包含 Operation/网络/数据库/权限/认证/错误/unknown 矩阵，三份
  Approval 均为 placeholder-only `draft / not approved`
- [x] 用户订阅 JSON 与独立油猴脚本缺失时明确阻塞，不猜造、不执行脚本，
  不以公开仓库历史脚本替代，不访问候选 Provider 地址
- [x] 只改授权文档，保持应用 1.1.0、Schema 4、Backup v2、依赖、配置、代码、
...
- [x] 新增 frozen/slots、tuple-only 的 VideoIdentifier、Person、Organization、
  Series、Tag、Rating、Asset、Provenance、SearchResult、Detail、SearchPage DTO
- [x] 新增 async `VideoMetadataAdapter` Protocol；search/detail/asset_list 不接受
  任意 URL/Host/Header/Auth 参数且严格分离
- [x] 新增有界文本、Provider-scoped identity、UTC、有限评分、opaque asset ID、
  available-fields/provenance 一致性和无 raw Mapping 保留的验证
- [x] 新增 tests-only `FixtureVideoMetadataProvider` 与六份合成静态 JSON fixture；
  不调用 DNS/socket/httpx/OutboundHttpClient，不进入 Production Registry
- [x] 新增零写入、确定性 `VideoMetadataMergePlan`：用户字段优先、缺失/空值不删除、
  provider priority 与 equal-priority conflict、Provider-scoped list 和 asset link
- [x] N4D-B 专项与 N4A/N4B/N4D-A/Adapter/Outbound 组合测试通过；应用/Schema/Backup、
  依赖、Registry、Docker/CI 与既有 data 边界保持不变

#### Phase 5-N4D-C - Provider Package 绑定与离线激活门禁

- [x] 新增 frozen/slots Evidence Manifest、Fixture Evidence、Adapter Binding、
  Provider Package 和稳定 `ProviderPackageErrorCode`
- [x] 精确绑定 Approval/Capabilities/Endpoint/Adapter/Evidence 的 Provider identity、
  display/content scope、scope 与 operation tuple
- [x] Adapter authority 只来自显式 binding；Source/Video Protocol 分开验证，额外方法
  不扩权，构造/验证/构建不执行 operation
- [x] static fixture evidence 使用 opaque ID 和固定 SHA-256；manifest 不含路径、raw
  response、环境变量、dynamic include 或可执行代码
- [x] Registry/binding builders 先验证全部 package，duplicate/第二包失败时 all-or-nothing；
  空 tuple 返回空结果且不修改 Production Registry
- [x] tests-only Source/Video synthetic packages 使用 `.invalid` hosts 和现有静态 fixtures；
  生产模块与门禁全程零网络/DNS/数据库/文件写入

#### Phase 5-N4D-D-A - Provider Approval Artifact v1 与离线装载门禁

- [x] 新增 frozen/slots Header、Adapter Ref、Attestation、Approval Artifact、Factory
  Binding/Registry 与 16 个稳定脱敏错误码
- [x] bytes-only strict JSON parser 拒绝 duplicate key、unknown/missing field、invalid
  UTF-8、bool/int 混淆、NaN/Infinity 与 depth/node/string/array/bytes 超限
- [x] canonical serializer 稳定排序、紧凑 Unicode、tuple/null round trip；SHA-256
  只覆盖不含 attestation 的 canonical payload，不实现签名/HMAC/密钥/远程信任
- [x] Artifact 精确恢复 Approval/Capabilities/Endpoint/Evidence/fixture digests，
  cross-component parity 在 factory lookup 前完成
- [x] opaque binding ID 禁止模块/class/path/URL；code-owned immutable registry 不使用
  importlib、entry point、环境或目录扫描，factory 前失败调用 0 次、成功只调用 1 次
- [x] factory 返回值继续经过 ProviderAdapterBinding、ProviderPackage 与 N4D-C validator；
  operation 不执行，异常与 Package cause 使用稳定脱敏错误
- [x] tests-only synthetic Video Artifact 只复用六份 `.invalid` fixture，不进入
  Production Registry；生产 parser/serializer/loader 零网络/DNS/数据库/文件写入

#### Phase 5-N4D-D-B0 - Repository-derived Provider Evidence Profile

- [x] 固定 JavdBviewed `8c9245726906ece8d49f553542874980512d4504`、JavSP
  `c4cfe61188234dd24c75b53b42b054327fef3e58`、FnDepot
  `9a2449eaf012c352bca2ed4381e005a37f67d757` 和 Venera
  `a0eba914f4c2a84ac1bc925adec2baabe920b9be`
- [x] 新增 Repository Evidence Ledger、Video Metadata Field Crosswalk、Operation
  Matrix、repository-derived profile v1 和 Production Readiness
- [x] JavSP 只提取 metadata/合并原则，JavdBviewed 只提取 local state/manual
  edit/soft delete/sync，FnDepot 只提取 manifest/versioning，Venera 只提取
  operation/pagination taxonomy
- [x] 四个仓库均标记为 reference-only 且不可直接激活；Approval draft
  仍为 `draft / not approved / no production activation`
- [x] Production Profile 仅保留 `search`、`detail`、可选 `asset_list`；Registry
  仍为 `EndpointRegistry(())`，无真实 Host/Endpoint/selector/response/凭据或网络
- [x] 仅修改授权 Markdown，未改生产代码、测试、版本、Schema、Backup、
...
- [ ] 用户明确批准 Provider 名称、核心用途、固定 Host/Endpoint、认证方式、
  search/detail/可选 asset_list、响应类型、限流、字段映射及条款边界
- [ ] 只实现该 Provider 明确需要并获授权的访问、认证、解析、DTO 映射和
  fixture/mock 测试；不开放任意 URL、通用 crawler 或隐藏网络

#### Phase 5-N4E - 订阅目录管理

- [ ] 静态审查实际订阅 envelope/schema 并批准固定 Catalog Host/Path；只允许
  用户主动 refresh/parse/revision/diff/approve/disable，GET 零网络
- [ ] 候选内容不直接注册、不访问 `baseUrl`；普通/`premium` 只分组，Host 变化
  重新批准，候选删除不删除本地数据或独立 runtime Provider

#### Phase 5-N4F - 在线播放 Provider

- [ ] 完成独立 Streaming Approval 后实现 playback_list/playback_resolve 与 UI，
  固定 Host/Manifest/Variant/Segment/auth/expiry/relative-reference 策略
- [ ] 播放生命周期、取消、失败、expired/unknown 完整；不移植绕权、免登录
  提取、油猴脚本或下载逻辑

#### Phase 5-N4G - 漫画 Provider

- [ ] 完成 Comic Approval 后以固定审查 Python Adapter 实现 search/detail/
  chapter_list/page_list；category/asset 必须单独纳入同一批准
- [ ] 保持 page/asset/locator、本地进度/远程状态分离，不执行 JavaScript，
  不自动整章下载或同步远程收藏

#### Phase 5-N5B - Search/detail empty-state 与 approved-provider UI

- [x] 新增登录保护的 GET `/source-search`、POST `/source-search/search` 与 POST
  `/source-search/detail`；GET 只调用 `list_providers()`，不执行 Provider operation
- [x] 新增 `get_provider_search_service`；生产只构造空 Package Service，测试仅通过
  dependency override 注入 tests-only Service，不从表单/环境/目录构造 Package
- [x] Search/Detail 分别通过 N5A request contract 和当前 descriptor catalog 前置
  复核；每次显式 POST 只调用对应 operation 一次，绝不自动链式调用
- [x] Production providers 为 `()` 时返回 HTTP 200 普通空状态，不显示 synthetic
  Provider、不创建业务 flash、不执行网络、数据库写入或文件写入
- [x] canonical URL 不链接，cover/preview/asset 不成为远程 img、播放或下载；Detail
  只显示非 locator Asset facts，响应不持久化到 DB/Session/Cookie/文件/缓存
- [x] 七类可渲染 Service error 映射为稳定脱敏状态与中英文提示，取消原样传播，
  query/external ID/cause/marker 不进入错误 HTML 或日志
...
- [x] 新增 `app/provider_apply/web.py`：Preview 按需创建 64 lowercase-hex Session nonce，
  Confirm 只读取既有 nonce；Session/app generation 精确匹配，logout/rotation 后旧 Token 失效
- [x] 使用 `SECRET_KEY` 与两个独立 HMAC-SHA256 domain 派生 exact 32-byte secret 和
  bounded opaque context；secret/context/nonce/generation 不进入模板、URL、flash、日志或 DB
- [x] Detail 只调用 Provider detail 一次，以 N5C-A bounded SELECT 只读构建 Plan；有写入时
  签发 600 秒 hidden Token 与 `no-store`，no-op 不签 Token、不显示表单、不创建 nonce
- [x] Preview 显示 create/update、safe field changes、keep-local、duplicate-title warning/link、
  10 分钟有效期、Confirm 零 Provider recall 与 stale 提示；不显示 URL/external ID/hash/key material
- [x] Confirm 精确校验 `confirmation=apply`，不读取 catalog，Provider operations 为 0，每请求
  最多调用 B1 apply 一次；成功 303 Item PRG，稳定中英文 flash 覆盖提交与全部失败类别
- [x] `commit_state_unknown` 不自动重试，303 到 Item 列表并明确要求先检查本地条目、禁止直接重提
- [x] GET 保持 Provider operation 0、DB call 0、nonce/Token/apply 0，production empty state 不变
- [x] focused `36 passed`、指定组合 `323 passed`、full `1388 passed`；Application `1.1.0`、
  Schema `4`、Backup v2 与空 Production Registry/Search Packages/Providers 保持
- [x] 未实现真实 Provider、认证/Vault、播放、下载、后台任务、Schema/Migration/依赖、
  Docker/Compose/CI、Hermes、Tag/Release 或 N100 部署；既有 `data/` 未接触
