...
- [ ] 在仓库外配置唯一强密码和随机 Secret，不使用示例占位值
- [ ] 在目标 N100 / LAN 主机验证 Docker build、启动、持久化、重启、登录和停止
- [ ] 使用真实桌面 / 移动端浏览器验证布局、JavaScript confirm 和详情访问记录 POST
- [ ] 导出当前 JSON 备份并通过校验
- [ ] 将备份恢复到全新隔离实例，核对核心实体、关系、设置和活动数量
- [ ] 确认服务仅在受控局域网访问，不直接暴露公网
- [ ] 记录最终无使用前阻断结论后再导入不可替代的真实数据
